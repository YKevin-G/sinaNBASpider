...
def write(path,text):
	with open(path,'a',encoding='utf-8') as f:
		f.writelines(text)
		f.write('\n')
def writetxt2py(path,pathpy):
	with open(path,'r') as fcontent:
		content = fcontent.readlines()
	alllinks = []
	for row in content:
		alllinks.append("'"+row.rstrip('\n')+"'")
		print(row.rstrip('\n'))
	with open(pathpy,'w') as fcontent:
		content = ','.join(alllinks)
		fcontent.write("ip = ["+content+"]")
